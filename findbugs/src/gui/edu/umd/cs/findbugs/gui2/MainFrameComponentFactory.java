...
//        mainFrame.getSummaryTopPanel().setMinimumSize(new Dimension(fontSize * 50, fontSize * 5));
    ...
    JPanel createSourceCodePanel() {
        ...
        //Font sourceFont = new Font("Monospaced", Font.BOLD, 20); //testing to see if I am editing the right section
        mainFrame.getSourceCodeTextPane().setFont(sourceFont);
        //mainFrame.getSourceCodeTextPane().setEditable(false);
        mainFrame.getSourceCodeTextPane().setEditable(true); //Changed to true by Zack Coker to allow copy-and-paste of text
        mainFrame.getSourceCodeTextPane().getCaret().setSelectionVisible(false);
    ...
    Component bugSummaryComponent(BugAnnotation value, BugInstance bug) {
        //JLabel label = new JLabel(); //Zack Coker - changing from JLabel to JTextField to allow selection of text
        //label.setFont(label.getFont().deriveFont(Driver.getFontSize()));
        //label.setFont(label.getFont().deriveFont(Font.PLAIN));
        //label.setForeground(Color.BLACK);
        JTextField textField = new JTextField();
        textField.setFont(textField.getFont().deriveFont(Driver.getFontSize()));
        ...
        if (value instanceof SourceLineAnnotation) {
            ...
            if (sourceCodeExists(link)) {
                ...
                textField.setToolTipText(clickToGoToText + " " + srcStr+": currently disabled for copy-paste ability");
            ...
            if (!srcStr.equals(sourceCodeLabel)){
                ...
                textField.setText(noteText + srcStr);
            }
            else {
            ...
            textField.setText(value.toString(primaryClass));
        }
