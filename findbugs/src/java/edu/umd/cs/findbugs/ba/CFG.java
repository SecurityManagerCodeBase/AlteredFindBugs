...
public class CFG extends AbstractGraph<Edge, BasicBlock> implements Debug {
     ...
     */
    public Edge createEdge(BasicBlock source, BasicBlock dest, @Edge.Type
    int type) {
     ...
     *         specified edge type
     */
    public BasicBlock getPredecessorWithEdgeType(BasicBlock target, @Type
    int edgeType) {
     ...
     *         specified edge type
     */
    public BasicBlock getSuccessorWithEdgeType(BasicBlock source, @Type
    int edgeType) {
     ...
     */
    public Edge getIncomingEdgeWithType(BasicBlock basicBlock, @Type
    int edgeType) {
     ...
     * @return the Edge, or null if there is no edge with that edge type
     */
    public Edge getOutgoingEdgeWithType(BasicBlock basicBlock, @Type
    int edgeType) {
        return getEdgeWithType(outgoingEdgeIterator(basicBlock), edgeType);
    }

    private Edge getEdgeWithType(Iterator<Edge> iter, @Type
    int edgeType) {
        while (iter.hasNext()) {
            Edge edge = iter.next();
    ...
    /*
     * A method added by Zack Coker to extract the previous locations that or
     * either the previous instruction location or the location of the last
     * instruction of other basic blocks with edges to the basic block of loc.
     * 
     * inputs: loc - the location to find the previousLocations of
     * 
     * returns: the collection of locations that precede the given location an
     * empty collection is returned if none are found
     */
    
    
    public Collection<LocationAndEdgeType> getPreviousLocations(Location loc) {
        LinkedList<LocationAndEdgeType> resultLocations = new LinkedList<LocationAndEdgeType>();
        InstructionHandle handle = loc.getHandle();
        BasicBlock basicBlock = loc.getBasicBlock();
        if (basicBlock.getFirstInstruction().equals(handle)) {
            Iterator<Edge> edgeIter = incomingEdgeIterator(basicBlock);
            // Set<Integer> edgeTypeSet = new HashSet<Integer>();
            BasicBlock prevBlock;
            while (edgeIter.hasNext()) {
                // edgeTypeSet.add(new Integer(edgeIter.next().getType()));
                Edge currentEdge = edgeIter.next();
                prevBlock = currentEdge.getSource();
                if (prevBlock != null ) { //preventing looping back to the same block
                    handle = prevBlock.getLastInstruction();
                    if (handle == null) {
                        Iterator<Edge> innerEdgeIter = incomingEdgeIterator(prevBlock);
                        while (handle == null) {
                            if (innerEdgeIter.hasNext()) {
                                currentEdge = innerEdgeIter.next();
                                prevBlock = currentEdge.getSource();
                                if (prevBlock != null) {
                                    handle = prevBlock.getLastInstruction();
                                }
                    ...
                    if (prevBlock == null)
                    {
                       prevBlock = loc.getBasicBlock(); // default return value if unable to find a new block - hopefully shouldn't happen
                    }
                    resultLocations.add(new LocationAndEdgeType(new Location(handle, prevBlock),currentEdge.getType()));
                    // break;

                }
            }

            // addLocationForEdgeType(basicBlock, EdgeTypes.FALL_THROUGH_EDGE,
            // resultLocations);
            // addLocationForEdgeType(basicBlock, EdgeTypes.GOTO_EDGE,
            // resultLocations);
            // addLocationForEdgeType(basicBlock, EdgeTypes.IFCMP_EDGE,
            // resultLocations);
            // addLocationForEdgeType(basicBlock,
            // EdgeTypes.HANDLED_EXCEPTION_EDGE, resultLocations);
            return resultLocations;

        } else {
            handle = handle.getPrev();
            resultLocations.add(new LocationAndEdgeType(new Location(handle, basicBlock)));
            return resultLocations;
    ...
    /*
     * private void addLocationForEdgeType(BasicBlock prevBlock, int edgeType,
     * Collection<Location> resultLocations) { InstructionHandle handle; while
     * (true) { prevBlock = getPredecessorWithEdgeType(prevBlock, edgeType); if
     * (prevBlock != null) { handle = prevBlock.getLastInstruction(); if (handle
     * != null) { resultLocations.add(new Location(handle, prevBlock)); break; }
     * } else { break; } } }
     */
